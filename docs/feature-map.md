...
Generated **2026-09-24** (America/Phoenix, MST) for local branch `grok/request-status-browser-verifier` based on main `342070c0e3c2e09205f4c8df38b3eb7f63644772`. Held-attempt recovery row added **2026-09-25** on `grok/held-request-recovery`; progress-consistency row added **2026-09-25** on `grok/progress-consistency`.
...
| **Progress consistency** | Tell at a glance whether Eve is working, waiting for my decision, blocked or done, the same way on every surface | Summary line (`p.summary[data-owner-state]`), ask panel button and notice, decision cards; Telegram `/status` | Owner session | One owner state (`ownerState`) from the same branch as "Next:". A submitted request is In progress immediately and after reload/refresh. Pending requests never invite resending. Proposals are never described as done. | `shared/status-summary.js`, `app/app.vue`, `hosted/steward.js` (`observedAt`), `hosted/telegram.js` | **Browser-verified:** `progress-consistency-working` (immediate, manual refresh, reload mid-judge, awaiting, completed), `progress-consistency-blocked`. **Controller:** `status-summary.test.js`, `telegram.test.js`. Docs: `docs/personal-pilot.md`. |
