...
import { telegramConfig, sendProgressNotification } from '../../hosted/telegram.js';
export default defineEventHandler(async event => {
  ...
  try {
    const { store, steward } = await getHostedRuntime();
    let notify = null;
    if (process.env.TELEGRAM_BOT_TOKEN) { const config = telegramConfig(); notify = job => sendProgressNotification(config, job); }
    return await new ProgressMonitor({ store, coding: steward.coding, notify }).run();
